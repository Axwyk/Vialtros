  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm px-4 py-4 flex flex-col gap-3 hover:shadow-md transition-shadow duration-200 cursor-default">
      <div className="flex items-start justify-between gap-2.5">
        <div className={`${c.iconBg} rounded-xl p-2 border border-white shadow-sm`}>
      ...
      <div>
        <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-[0.2em]">{label}</p>
        <p className="mt-1.5 text-[2rem] font-bold tracking-tight text-slate-900 leading-none">{value}</p>
      </div>
    </div>
  );
}
